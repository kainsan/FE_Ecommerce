...
const AdminUserComponent = () => {
  ...
  const { data: dataDeletedMany, isLoading: isPendingDeletedMany, isSuccess: isSuccessDeletedMany, isError: isErrorDeletedMany } = mutationDeletedMany
      ...
      >
        ...
        <Space>
          <Button
            ...
            size="small"
            style={{
              width: 90,backgroundColor: '#4096ff'
            }}
          >
            Search
          </Button>
  ...
  return (
    <div>
      <h1 className="text-[#000] text-sm">Manage User</h1>
      <div style={{ marginTop: '20px' }}>
        <TableComponent handleDeleteMany={handleDeleteManyUser} isPending={isPendingUsers} columns={columns} data={dataTable} onRow={(record, rowIndex) => {
          return {
            onClick: event => {
      ...
      <DrawerComponent title='Chi tiết user' isOpen={isOpenDrawer} onClose={() => setIsOpenDrawer(false)} width="90%">
        <Pending isPending={isLoadingUpdate || isPendingUpdated}>
          <Form
            name="basic"
            labelCol={{ span: 2 }}
            wrapperCol={{ span: 22 }}
      ...
      <ModalComponent forceRender title="Xóa user" open={isModalOpenDelete} onCancel={handleCancelDelete} onOk={handleDeleteUser} okButtonProps={{ style: { backgroundColor: '#4096ff' } }}>
